...
namespace orp
{
	namespace calibration
	{
		...
		*/
			...
			cv::Mat& outputImage) 
		{
			...
			for (int r = 1; r < rows - 1; ++r) 
			{
				...
				for (int c = 1; c < cols - 1; ++c) 
				{
					...
					double Lxx = (v21 - 2.0 * v22 + v23);
					double Lyy = (v12 - 2.0 * v22 + v32);
		...
		void drawCheckerboardCorners(cv::Mat& result, const BoardObservation& obs, int thickness, bool triangular_board)
		{
			cv::Mat a = cv::Mat(obs.board.rows, 1, CV_8UC3);
			
			//print_debug(obs.board.rows);
			for (int i = 0; i < obs.board.rows; i++)
			{
				float step = 255.f / (obs.board.rows - 1)*3.5f;
				a.at<uchar>(i, 0) = uchar(cv::saturate_cast<int>(step * i)%256);
				a.at<uchar>(i, 1) = 255;
				a.at<uchar>(i, 2) = 255;
			}
			cv::cvtColor(a, a, cv::COLOR_HSV2BGR_FULL);
			...
			const int line_max = obs.board.rows;
			cv::AutoBuffer<cv::Scalar>line_colors(line_max);
			for (int i = 0; i < line_max; i++)
			{
				//std::cout << (int)a.at<uchar>(i, 0)<<","<<(int) a.at<uchar>(i, 1)<<","<<(int) a.at<uchar>(i, 2) << std::endl;
				line_colors[i][0] = a.at<uchar>(i, 0);
				line_colors[i][1] = a.at<uchar>(i, 1);
				line_colors[i][2] = a.at<uchar>(i, 2);
			}
				...
				for (int c = 0; c < checkerboard.cols; c++)
				{
					...
					if (c + 1 < checkerboard.cols)
					{
						...
						if (id1 != -1 && id2 != -1)
						{
							...
							/*print_debug2(id1, corners[id1]);
							print_debug2(id2, corners[id2]);
							cv::imshow("test", result); cv::waitKey();*/
							if (obs.indexed)
							{
								std::cout << "obs.indexed: true" << std::endl;
								subpixel_marker(result, corners[id1], rs, color, thickness);
								subpixel_marker(result, corners[id2], rs, color, thickness);
								if (!index_shown)
								{
