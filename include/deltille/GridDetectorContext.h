...
namespace orp
{
	...
	{
		template <typename SaddlePointType, typename InputImageType = uint8_t, typename FloatImageType = float>
		...
		private:
			...
			PolynomialSaddleDetectorContext<SaddlePointType, InputImageType, FloatImageType> detector;
		...
		public:
			...
			bool precomputePolaritiesAndNN(const SaddlePointVector& points)
			{
				...
				{
					pts[i].computePolarities(polarity.ptr<typename SaddlePointType::PolarityStorageType>(i));
				}
				...
				for (int i = 0; i < num_pts; ++i)
				{
					...
					partial_sort_indexes(dist.ptr<double>(i), NN.ptr<int>(i), pts.size(), max_nn);
			...
			bool growGrid(std::vector<CoordIndex>& coords, std::vector<bool>& visited, cv::Mat& idxmap, int offr, int offc)
			{
				int added = 0;
			...
			{
				//print_matinfo_detail(active);
				cv::Scalar total = cv::sum(active);
				if (total[0] < 16) return false;

				//std::cout << "findBestGrid: " << board_size << std::endl;
				//std::cout << "board_size.area()" << board_size.area() << std::endl;
				if (board_size.area() == 0) return false;
			...
			int findBoards(const cv::Size& board_size, std::vector<BoardObservation>& boards, bool best_only)
			{
				...
				bool flag = precomputePolaritiesAndNN(refclust);
				...
				while (true)
				{
...
#endif
					bool flag = findBestGrid(board_size, obs.board);

					//if (flag)
					/*{
						if (this->deltilleGrid)std::cout << "deltilleGrid: true" << std::endl;
						else std::cout << "deltilleGrid: false" << std::endl;
						std::cout << "obs.board.cols " << obs.board.cols << std::endl;
						std::cout << "obs.board.rows " << obs.board.rows << std::endl;
						print_matinfo(obs.board);
					...
					for (int r = 0; r < obs.board.rows; ++r)
					{
						for (int c = 0; c < obs.board.cols; ++c)
						{
							...
							{
								corners.push_back(cv::Point2f(std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity()));
							}
						}
					}
