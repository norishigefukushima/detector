		...
		template <typename SaddlePointType, typename InputImageType = uint8_t,
			...
			public:
				...
				int findSaddles(std::vector<SaddlePointType>& refclust)
				{
					...
					if (!SaddlePointType::isTriangular)
					{
						...
						const double min_angle_width = 15.0 * M_PI / 180.0;
						const double max_angle_width = 75.0 * M_PI / 180.0;
					...
					if (SaddlePointType::isTriangular)
					{
						...
						cv::Mat temp;
						cv::filter2D(input_lowres, temp, cv::DataType<FloatImageType>::depth, tempFitting.getSmoothingKernel());
			...
			private:
				...
				void preprocessImage(cv::InputArray img)
				{
					...
					cv::filter2D(input_lowres, lowres, cv::DataType<FloatImageType>::depth, lowresFitting.getSmoothingKernel());
					cv::Mat temp; lowres.convertTo(temp, CV_8U);
				}
					...
					std::vector<cv::Point>& locations)
				{
					...
					double gsigma = 1.5;
					//cv::GaussianBlur(gauss_img, gauss_img, cv::Size(ceil(gsigma*3)*2+1, ceil(gsigma * 3) * 2 + 1), gsigma);
