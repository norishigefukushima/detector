...
namespace orp
{
	namespace calibration
	{
		...
		template <typename FloatPoint>
		void subpixel_line(cv::Mat& img, const FloatPoint& pt1, const FloatPoint& pt2, const cv::Scalar& color, int thickness = 1, int lineType = cv::LINE_AA)
		{
			const int shift = 16;
			const float ss = (1 << shift);
			//std::cout << cv::Point(int(pt1.x * ss), int(pt1.y * ss)) << "to" << cv::Point(int(pt2.x * ss), int(pt2.y * ss)) << std::endl;
			cv::line(img, cv::Point(int(pt1.x * ss), int(pt1.y * ss)), cv::Point(int(pt2.x * ss), int(pt2.y * ss)), color, thickness, lineType, shift);
		}
