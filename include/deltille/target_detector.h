...
struct CalibrationCorner
{
	...
	CalibrationCorner(double x_ = -1.0, double y_ = -1.0, int b_id = -1, int p_id = -1, int o = 0)
...
private:
	...
	friend std::ostream& operator<<(std::ostream& os, const CalibrationCorner& c)
	{
...
template <class SaddlePointType, class FloatImageType = double, typename ImageType = uint8_t>
static inline int FindBoardsHelper(const cv::Mat& image, const cv::Size& board_size, std::vector<orp::calibration::BoardObservation>& boards)
{
	boards.clear();
	orp::calibration::GridDetectorContext<SaddlePointType, ImageType, FloatImageType> gd(image);
...
static inline int FindBoards(const cv::Mat& I, const cv::Size& board_size, std::vector<orp::calibration::BoardObservation>& boards)
{
	switch (I.type())
	{
...
private:
	...
	{
		return !_indexer.board_defs.empty() && _indexer.board_defs.front().triangular;
	}
};
